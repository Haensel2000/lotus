...
struct FONT3DVERTEXC { D3DXVECTOR3 p;   DWORD color;     FLOAT tu, tv; };

#define D3DFVF_FONT2DVERTEX (D3DFVF_XYZRHW|D3DFVF_DIFFUSE|D3DFVF_TEX1)
#define D3DFVF_FONT3DVERTEX (D3DFVF_XYZ|D3DFVF_NORMAL|D3DFVF_TEX1)
#define D3DFVF_FONT3DVERTEXC (D3DFVF_XYZ|D3DFVF_DIFFUSE|D3DFVF_TEX1)
...
// eigene Erweiterung
inline FONT3DVERTEXC InitFont3DVertexC( const D3DXVECTOR3& p, const D3DCOLOR color,
                                      FLOAT tu, FLOAT tv )
{
    FONT3DVERTEXC v;   v.p = p;   v.color = color;   v.tu = tu;   v.tv = tv;
    return v;
}
...
//-----------------------------------------------------------------------------
HRESULT CD3DFont::DrawTextScaled( long screenwidth, long screenheight, FLOAT x, FLOAT y, FLOAT z,
...
//    m_pd3dDevice->GetViewport( &vp );
    ...
    if( dwFlags & D3DFONT_CENTERED_X )
    {
        ...
        while( *strTextTmp )
        {
            ...
            w *= (fXScale*screenheight)/fLineHeight;

            //xFinal += w - (2 * m_dwSpacing) * (fXScale*vp.Height)/fLineHeight;
            xFinal += w - (2 * m_dwSpacing) * (fXScale*screenheight)/fLineHeight;
        }

        //x = -xFinal/vp.Width;
        x = -xFinal/screenwidth;
    }
    if( dwFlags & D3DFONT_CENTERED_Y )
    {
        //y = -fLineHeight/vp.Height;
        y = -fLineHeight/screenheight;
    }

    //FLOAT sx  = (x+1.0f)*vp.Width/2;
    //FLOAT sy  = (y+1.0f)*vp.Height/2;
    FLOAT sx  = (x+1.0f)*screenwidth/2;
    FLOAT sy  = (y+1.0f)*screenheight/2;
    FLOAT sz  = z;
    FLOAT rhw = 1.0f;

    // Adjust for character spacing
    //sx -= m_dwSpacing * (fXScale*vp.Height)/fLineHeight;
    sx -= m_dwSpacing * (fXScale*screenheight)/fLineHeight;
    FLOAT fStartX = sx;
    ...
    while( *strText )
    {
        ...
        w *= (fXScale*screenheight)/fLineHeight;
        h *= (fYScale*screenheight)/fLineHeight;
        ...
        sx += w - (2 * m_dwSpacing) * (fXScale*screenheight)/fLineHeight;
    }
...
//-----------------------------------------------------------------------------
HRESULT CD3DFont::DrawText( long screenwidth, long screenheight, FLOAT sx, FLOAT sy, DWORD dwColor,
                            const TCHAR* strText, DWORD dwFlags )
{
    if( m_pd3dDevice == NULL )
        return E_FAIL;
...
//-----------------------------------------------------------------------------
// Name: Render3DTextColored()
// Desc: Renders 3D text coloriert; zusätzliche Funktion von Alexander Rudolph
//-----------------------------------------------------------------------------
HRESULT CD3DFont::Render3DTextColored(const TCHAR* strText, D3DCOLOR Color,  DWORD dwFlags )
{
    if( m_pd3dDevice == NULL )
        return E_FAIL;
    ...
    m_pStateBlockSaved->Capture();
    m_pStateBlockDrawText->Apply();
    m_pd3dDevice->SetFVF( D3DFVF_FONT3DVERTEXC );
    m_pd3dDevice->SetPixelShader( NULL );
    m_pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof(FONT3DVERTEXC) );
    ...
    while( (c = *strText++) != 0 )
    {
        ...
        if( c != _T(' ') )
        {
            *pVertices++ = InitFont3DVertexC( D3DXVECTOR3(x+0,y+0,0), Color, tx1, ty2 );
            *pVertices++ = InitFont3DVertexC( D3DXVECTOR3(x+0,y+h,0), Color, tx1, ty1 );
            *pVertices++ = InitFont3DVertexC( D3DXVECTOR3(x+w,y+0,0), Color, tx2, ty2 );
            *pVertices++ = InitFont3DVertexC( D3DXVECTOR3(x+w,y+h,0), Color, tx2, ty1 );
            *pVertices++ = InitFont3DVertexC( D3DXVECTOR3(x+w,y+0,0), Color, tx2, ty2 );
            *pVertices++ = InitFont3DVertexC( D3DXVECTOR3(x+0,y+h,0), Color, tx1, ty1 );
            dwNumTriangles += 2;
