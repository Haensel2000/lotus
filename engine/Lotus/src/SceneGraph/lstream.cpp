...
bool Stream::s_registered = false;
HashTable<String, FactoryFunction> Stream::s_factory(Stream::FACTORY_SIZE);
const String Stream::ROOT = "$ROOT$";

/* ------------------------------------------------------- */
/* class Stream :                                          */
/*                      PUBLIC                             */
/* ------------------------------------------------------- */
Stream::Stream(Renderer* renderer, int bufferSize) 
: m_topLevel(SET_SIZE, SET_GROW), m_objectLinks(0), m_sharedLinks(0), m_objects(0),
	m_input(0), m_output(0), m_bufferSize(bufferSize), m_renderer(renderer)
{
...
/* ------------------------------------------------------- */
bool Stream::add(Spatial* obj)
{
	return m_topLevel.append(obj);
}
/* ------------------------------------------------------- */
bool Stream::contains(Spatial* obj)
{
	return m_topLevel.contains(obj);
}
/* ------------------------------------------------------- */
bool Stream::remove(Spatial* obj)
{
	return m_topLevel.removeObject(obj);
}
/* ------------------------------------------------------- */
void Stream::removeAll()
{
	m_topLevel.removeAll();
}
/* ------------------------------------------------------- */
...
/* ------------------------------------------------------- */
Set<Serializable*>& Stream::getObjects()
{
	return m_topLevel;
}
/* ------------------------------------------------------- */
...
void Stream::readBuffer()
{
	...
	fread(m_buffer + rest, sizeof(char), diskRead, m_input);
}
/* ------------------------------------------------------- */
void Stream::writeBuffer()
{
	...
	{
		fwrite(m_buffer, sizeof(char), m_localBufferPos, m_output);
		m_localBufferPos = 0;
	}
}
/* ------------------------------------------------------- */
